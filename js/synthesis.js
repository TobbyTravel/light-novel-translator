...
export async function runSynthesis({ projectId, settings, onProgress, onToken, signal }) {
  ...
  await startJob(projectId, { kind: 'synthesis', stage: plan.mode, totalBatches: plan.calls });

  let result;
  if (plan.mode === 'single') {
    onProgress?.({ stage: 'single', batch: 1, totalBatches: 1 });
    const call = await callSynthesis({ settings, system, prompt: synthesisUserPrompt({ timelineEntries: entries }), onToken, signal });
    result = call.result;
    await updateJob(projectId, { stage: 'single', batchIndex: 1, addTokensIn: call.promptTokens ?? 0, addTokensOut: call.completionTokens ?? 0 });
  } else {
    const batches = batchEntries(entries, system, settings.contextBudget);
    const partials = [];
    for (let i = 0; i < batches.length; i++) {
      if (signal?.aborted) {
        await finishJob(projectId, 'cancelled');
        throw Object.assign(new Error('Aborted'), { name: 'AbortError' });
      }
      onProgress?.({ stage: 'map', batch: i + 1, totalBatches: batches.length });
      await updateJob(projectId, { stage: 'map', batchIndex: i });
      const call = await callSynthesis({ settings, system, prompt: synthesisUserPrompt({ timelineEntries: batches[i] }), onToken, signal });
      partials.push(call.result);
      await updateJob(projectId, { addTokensIn: call.promptTokens ?? 0, addTokensOut: call.completionTokens ?? 0 });
    }
    onProgress?.({ stage: 'reduce', batch: batches.length + 1, totalBatches: batches.length + 1 });
    await updateJob(projectId, { stage: 'reduce', batchIndex: batches.length });
    try {
      const call = await callSynthesis({ settings, system, prompt: synthesisReducePrompt({ partialSyntheses: partials }), onToken, signal });
      result = call.result;
      await updateJob(projectId, { addTokensIn: call.promptTokens ?? 0, addTokensOut: call.completionTokens ?? 0 });
    } catch (err) {
  ...
  await finishJob(projectId, 'done');
  return record;
}
